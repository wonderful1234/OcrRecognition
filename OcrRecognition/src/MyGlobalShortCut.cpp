...
MyGlobalShortCut::MyGlobalShortCut(QString key, QWidget *parent, bool reportError)
{
    QStringList keys = { "A","S", "P", "V" };
    for (auto key : keys)
    {
		QString keycode = "ctrl+shift";
        keycode += "+";
        keycode += key;
		m_key = QKeySequence(keycode);
        if (registerHotKey())
        {
